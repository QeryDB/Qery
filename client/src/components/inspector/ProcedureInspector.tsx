...
export function ProcedureInspector({ connectionId, database, name, schema, definition: propDefinition, breadcrumb = [], tabId }: Props) {
  const { t } = useTranslation();
  const tabKey = `proc-${connectionId}-${database}-${schema}-${name}`;
  const [activeSubTab, setActiveSubTab] = useState(() => getSubTab('proc', tabKey, connectionId, database, schema, name, 'definition'));
  const handleSubTabChange = useCallback((value: string) => {
    setActiveSubTab(value);
    setSubTab('proc', tabKey, value, connectionId, database, schema, name);
  ...
  // Fallback: parse from definition (MSSQL @param syntax)
  ...
  const currentTarget: InspectorTarget = { connectionId, database, table: name, schema, objectType: 'procedure' };
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <InspectorHeader name={`${schema}.${name}`} connectionId={connectionId} database={database} typeBadge={{ label: 'PROCEDURE', color: '#f97316' }} />
      <Tabs value={activeSubTab} onValueChange={handleSubTabChange} className="flex flex-1 flex-col min-h-0">
        <ScrollableTabsList className={tabListCls}>
          ...
          <TabsTrigger value="execute" className={tabTriggerCls}>{t("editor.execute")}</TabsTrigger>
          <TabsTrigger value="parameters" className={tabTriggerCls}>
            {t('editor.variables')} ({inputCount}{varCount > 0 ? `+${varCount}` : ''})
          </TabsTrigger>
